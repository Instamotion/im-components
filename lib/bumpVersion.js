...
function bumpDependencyVersion(pkg, depName, depType, newDepVer) {
  console.log(`Bumping dep in ${pkg.name} : ${pkg[depType][depName]} -> ${newDepVer}`);
  pkg[depType][depName] = newDepVer;

  const newPkgVer = semver.inc(pkg.version, 'patch');
  console.log(`... Patching ${pkg.name}: ${newPkgVer}`);
  ...
  return query({
    ...
    .forEach(p => {
      ...
      if (pkg.dependencies && pkg.dependencies[pkgName]) {
        bumpDependencyVersion(pkg, pkgName, 'dependencies', newVer);
      }

      if (pkg.devDependencies && pkg.devDependencies[pkgName]) {
        bumpDependencyVersion(pkg, pkgName, 'devDependencies', newVer);
      }

      if (pkg.peerDependencies && pkg.peerDependencies[pkgName]) {
        bumpDependencyVersion(pkg, pkgName, 'peerDependencies', newVer);
      }
